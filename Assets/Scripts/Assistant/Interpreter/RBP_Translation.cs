...
namespace T2G
{
    public class RBP_Translation : Translation     //Rule-Based Process
    {
        ...
        static (string pattern, string key)[] _rules = {
            ...
            (@"^(enter|go to)\s+(?:(?<type>scene|level|space)\s+)?(?:named|with the name\s+)?(?<name>.+?)(?:\.)?$", "enter_space"),
        ...
        static int UpdateBestMatch(Match match, int index)
        {
        ...
        public static (int index, int score)[] TestRegexMatch(string text)
        {
            _bestMatchIndex = -1;
            _bestMatchScore = -1;
            _bestMatch = null;

            List<(int, int)> matchIndices = new List<(int, int)>();
            for(int i = 0; i < _rules.Length; ++i)
            {
                ...
                if(match.Success)
                {
                    int score = UpdateBestMatch(match, i);
                    matchIndices.Add((i, score));
                }
            }
        ...
        protected override bool ParseInstructionData(string prompt, out string key, out (string name, string value)[] arguments)
        {
            ...
            for(int i = 0; i < _rules.Length; ++i)   
                //TODO: consider using multi-thread for betterr performance when the list count is large
            {
                var pattern = _rules[i].pattern;
